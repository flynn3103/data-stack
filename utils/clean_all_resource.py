    ...
    def delete_eventbridge_rules(self, region_name):
        ...
        for event_bus_name in bus_names:
            ...
            for rule_name in rule_names:
                ...
                if target_ids:
                    try:
                        events.remove_targets(Rule=rule_name, EventBusName=event_bus_name, Ids=target_ids, Force=True)
                    except Exception as e:
                        logger.info(f"Info: removing targets for rule {rule_name} on event bus {event_bus_name}: {e}")
                        continue
                try:
                    events.delete_rule(Name=rule_name, EventBusName=event_bus_name,  Force=True)
